...
describe("upgrade command", () => {
  ...
  test("rebuilds artifacts and reinstalls the selected agent integration", () => {
    ...
    expect(result.stdout).toContain("Updated codemem project resources");
    ...
    expect(existsSync(join(homeDir, ".local", "bin", "codemem"))).toBe(false);
    expect(existsSync(join(homeDir, ".codemem", "_system", "install.json"))).toBe(false);
  }, 30000);
