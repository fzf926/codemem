...
推荐你直接使用一键安装脚本。脚本会自动拉取 `codemem` 项目、构建 CLI、写入全局 `codemem` 命令，把命令目录加入 shell profile，并安装对应 agent 集成。

如果脚本已经发布在 GitHub，你可以在目标项目目录直接执行：
